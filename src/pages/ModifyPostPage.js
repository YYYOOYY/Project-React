...
function ModifyPostPage() {
    ...
    const findPostHandle = () => {
        ...
        if (xhr.status === 200) {
            ...
            formRef.current.title.value = body.post.title;
            formRef.current.content.value = body.post.content.replace(/<br>/g, '\n');
            formRef.current.cate.value = body.post.cate;
            formRef.current.cate.value = body.post.cate;
            formRef.current.city.value = body.post.city;
            formRef.current.price.value = body.post.price;
            setImageUrls(body.post.photos.map(item => item.imageUrl));
        }
    }
    ...
    const submitHandle = () => {
        ...
        const xhr = new XMLHttpRequest();
        xhr.open("PATCH", REST_SERVER_ADDRESS + `/post/update/${postId}`, false);
        xhr.setRequestHeader("Authorization", jwt);
